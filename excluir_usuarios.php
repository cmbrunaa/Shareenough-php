...
if (isset($_GET['excluir']) && isset($_GET['tipo'])) {
    ...
    $stmt->execute();
    echo "<script>alert('Registro excluído com sucesso!'); window.location.href='admin.php';</script>";
}
?>
        ...
        <div class="container my-5" id="admexcmain">
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if ($stmt->rowCount() == 0) {
                        echo "<tr><td colspan='7' class='text-center'>Não há usuários cadastrados.</td></tr>";
                    } else {
                        while ($user = $stmt->fetch()) {
                            echo "<tr>";
                            echo "<td><img src='{$user['imagem']}' alt='Imagem do Usuário' style='width:50px;height:50px;'></td>"; // Exibir imagem do usuário
                            ...
                            echo "<td>
                                    <a href='admin.php?excluir=1&id={$user['id_usuario']}&tipo=usuario' onclick=\"return confirm('Deseja realmente excluir este usuário?');\"><button class='btn-exc-adm'>Excluir</button></a>
                                  </td>";
                            echo "</tr>";
                        }
                    }
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if ($stmt->rowCount() == 0) {
                        echo "<tr><td colspan='8' class='text-center'>Não há ONGs cadastradas.</td></tr>";
                    } else {
                        while ($ong = $stmt->fetch()) {
                            echo "<tr>";
                            echo "<td><img src='{$ong['foto']}' alt='Foto da ONG' style='width:50px;height:50px;'></td>"; // Exibir foto da ONG
                            ...
                            echo "<td>
                                    <a href='admin.php?excluir=1&id={$ong['id_ong']}&tipo=ong' onclick=\"return confirm('Deseja realmente excluir esta ONG?');\"><button class='btn-exc-adm'>Excluir</button></a>
                                  </td>";
                            echo "</tr>";
                        }
                    }
